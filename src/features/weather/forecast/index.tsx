...
export default function ForecastSection(): JSX.Element {
	...
	return (
		<Section
			type='default'
			title={name.toLocaleLowerCase() + ', ' + weatherData.city.country.toLocaleLowerCase()}>
			<ul className='forecast'>
				{weatherData.list.slice(sliceStart, sliceStart + 8).map((weatherElement, index) => {
					...
					return (
						<li
							key={index}
							className='forecastElement'>
							<ul>
								<li>{index === 0 && forecastIndex === 0 ? 'Now' : hour}</li>
								<li>
									<img
										src={getWeaterStatusLink(weatherElement.weather[0].icon)}
									/>
								</li>
								<li>{temp}°</li>
							</ul>
			...
			<button
				...
				onClick={() => setForecastIndex(0)}>
				today
			</button>
			<button
				className='weekday'
				disabled={forecastIndex === 1}
				onClick={() => setForecastIndex(1)}>
				{weekDays[(todayIndex + 1) % 7]}
			</button>
			<button
				className='weekday'
				disabled={forecastIndex === 2}
				onClick={() => setForecastIndex(2)}>
				{weekDays[(todayIndex + 2) % 7]}
			</button>
			<button
				className='weekday'
				disabled={forecastIndex === 3}
				onClick={() => setForecastIndex(3)}>
				{weekDays[(todayIndex + 3) % 7]}
			</button>
			<button
				className='weekday'
				disabled={forecastIndex === 4}
				onClick={() => setForecastIndex(4)}>
