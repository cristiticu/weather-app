...
export function useSuggestionHandling(onSubmit: Function, onError: Function) {
	...
	const handleCityChanged = debounce(async (e) => {
		if (e.target.value.length >= 3) {
			try {
				const { data: coordinateData } = await coordinateTrigger(
					{ city: e.target.value, limit: 3 },
					true
				);

				setSuggestions(
					coordinateData.map((newSuggestion) => {
						return {
							name: newSuggestion.name.toLocaleLowerCase(),
							country: newSuggestion.country.toLocaleLowerCase(),
							coords: { lat: newSuggestion.lat, long: newSuggestion.lon },
						};
					})
				);
				setError('');
