	...
	function handleLocalization() {
		async function onSucces(position: GeolocationPosition) {
			...
			const response = await fetch(
				`https://api.openweathermap.org/geo/1.0/reverse?lat=${position.coords.latitude}&lon=${position.coords.longitude}&limit=1&appid=8eb16d0f89f9abb9566d44e84d13627f`
			);
			const fetchedCity = await response.json();

			const positionData = {
				name: fetchedCity[0].name,
				country: fetchedCity[0].country,
				coords: {
