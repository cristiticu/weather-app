...
export default function WeatherSection({ weatherData }): JSX.Element{
    ...
    const windSpeedDescriptions = ['still winds', 'light winds', 'light breeze', 
                                    'gentle breeze', 'fresh breeze', 'strong breeze',
                                    'moderate gale', 'fresh gale', 'strong gale',
                                    'storm', 'hurricane'];
    const windSpeedIndex = Math.floor((windSpeed > 100 ? 100 : windSpeed) / 10);

    return (
        <Section type='default' title={weatherData.name.toLowerCase() + ', ' + weatherData.sys.country.toLowerCase()}>
